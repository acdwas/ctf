...
class Flawless:
	...
	def move(self, c, x, cipher):
		...
		for cc in range(0x20, 0x7f):
			for i in range(len(self.alphabet)):
				self.alphabet[i] = chr((cc ^ ord(self.alphabet[i])) % 0xff)
			i = printable.index(c)
			if self.alphabet[i] == chr(cipher[x]):
				self.w += chr(cc)
				break
			self.alphabet = tmp[:]
		self.current += 1
	...
	def press(self, c, x, cipher):
		if self.P > 0 and self.I > 0 and self.N > 0 and self.G > 0:
			self.move_left()
			self.reset_ping()
		elif ord(c) % 4 == 0:
			self.move(c, x, cipher)
		elif ord(c) % 5 == 0:
			self.move_right()
		i = printable.index(c)
		return self.alphabet[i]
	
	def encipher(self, text, cipher):
		self.current = 0
		self.reset_ping()
		ciphertext = ""
		# for c in text:
		for i, c in enumerate(text):
			if c.upper() == 'P':
				...
				self.G += 1
			ciphertext += self.press(c, i, cipher)
...
enciphered = generator.encipher(very_secret_cipher_text, bytes.fromhex(l))
